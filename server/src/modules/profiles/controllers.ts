...
export const updateProfileController = async (req: Request, res: Response) => {
  try {
    ...
    if (authenticatedUserId !== userId) {
      return res
        .status(403)
        .json({ error: "Unauthorized: Cannot update another user's profile" });
    }
...
export const deleteProfileController = async (req: Request, res: Response) => {
  try {
    ...
    if (authenticatedUserId !== userId) {
      return res
        .status(403)
        .json({ error: "Unauthorized: Cannot delete another user's profile" });
    }
...
export const domainMapController = async (req: Request, res: Response) => {
  try {
    ...
    if (!result)
      return res
        .status(404)
        .json({ error: "No university found for this email domain" });

    res.json(result);
  } catch (error) {
    logger.error("Error mapping domain", {
      error: error instanceof Error ? error.message : String(error),
    });
