...
describe("Likes API happy path cases", () => {
      ...
      cb({
        likes: {
          findFirst: jest
            ...
            .mockResolvedValueOnce(sampleLike),
          create: jest.fn().mockResolvedValue(sampleLike),
        },
        matches: {
  ...
  it("should check if two users are matched", async () => {
    (prisma.matches.findFirst as jest.Mock).mockResolvedValue({
      id: "match-id",
    });
...
describe("Likes API edge & failure cases", () => {
  ...
  it("should throw error when user tries to like themselves", async () => {
    await expect(
      LikesService.createLike({ from_user: "same-user", to_user: "same-user" }),
  ...
  it("should throw error when user profiles are not found", async () => {
    (prisma.profiles.findUnique as jest.Mock)
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null);
    await expect(LikesService.createLike(sampleLike)).rejects.toThrow(
  ...
  it("should throw error when removing a like not owned by user", async () => {
    ...
    await expect(LikesService.removeLike("like-id", fromUser)).rejects.toThrow(
      "Like not found or unauthorized",
    );
  });
  ...
  it("should throw an error if from_user is missing", async () => {
    await expect(
      LikesService.createLike({ from_user: "", to_user: "user2" }),
    ).rejects.toThrow("Both from_user and to_user are required");
  });
  ...
  it("should throw an error if users are from different universities", async () => {
    ...
    await expect(LikesService.createLike(sampleLike)).rejects.toThrow(
      "Users must be from the same university",
    );
  });
  ...
  it("should throw an error if the daily superlike limit has been reached", async () => {
    ...
    await expect(
      LikesService.createLike({ ...sampleLike, is_superlike: true }),
    ).rejects.toThrow("Daily superlike limit of 5 reached");
  ...
  it("should throw an error if userId is not provided to getSentLikes", async () => {
    await expect(LikesService.getSentLikes("")).rejects.toThrow(
      "User ID is required",
    );
  });

  it("should throw an error if userId is not provided to getReceivedLikes", async () => {
    await expect(LikesService.getReceivedLikes("")).rejects.toThrow(
      "User ID is required",
    );
  });
  ...
  it("should throw an error if likeId is not provided to removeLike", async () => {
    await expect(LikesService.removeLike("", fromUser)).rejects.toThrow(
      "Like ID and User ID are required",
    );
  });
});
...
describe("getReceivedLikesCount", () => {
  ...
  it("should throw an error if userId is not provided", async () => {
    await expect(LikesService.getReceivedLikesCount("")).rejects.toThrow(
      "User ID is required",
    );
  });
});
...
describe("getTodaysSuperlikeCount", () => {
  ...
  it("should throw an error if userId is not provided", async () => {
    await expect(LikesService.getTodaysSuperlikeCount("")).rejects.toThrow(
      "User ID is required",
    );
  });
});
