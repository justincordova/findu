...
describe("Likes API happy path cases", () => {
  ...
  it("should create a like successfully without creating a match", async () => {
    (prisma.profiles.findUnique as jest.Mock).mockResolvedValue({ university: "Uni", user_id: fromUser });
    ...
    (prisma.$transaction as jest.Mock).mockImplementation(async (cb: any) =>
      cb({
        ...
        matches: {
          findFirst: jest.fn().mockResolvedValue(null),
          create: jest.fn().mockResolvedValue({ id: "match-id" }),
        },
      })
    );
    ...
    (prisma.$transaction as jest.Mock).mockImplementation(async (cb: any) =>
      cb({
        likes: {
          findFirst: jest
            .fn()
            .mockResolvedValueOnce(null) // check for existing like
            .mockResolvedValueOnce(sampleLike), // reciprocal
          create: jest.fn().mockResolvedValue(sampleLike),
          count: jest.fn().mockResolvedValue(0),
        },
        matches: {
