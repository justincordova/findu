...
} from "@/modules/discover/services";

async function debugDiscoverAlgorithm(
  viewerEmail: string,
  ...
  try {
    ...
    console.log(
      `   Gender Preference: ${JSON.stringify(viewerProfile.gender_preference)}`,
    );
    console.log(
      `   Age Preference: ${viewerProfile.min_age}-${viewerProfile.max_age}`,
    );
    ...
    if (!candidateUser?.profiles) {
      console.error(
        `❌ Candidate ${candidateEmail} not found or has no profile`,
      );
      return;
    }
    ...
    console.log(
      `   Age: ${calculateAge(candidateProfile.birthdate)} years old`,
    );
    console.log(
      `   Gender Preference: ${JSON.stringify(candidateProfile.gender_preference)}`,
    );
    console.log(
      `   Age Preference: ${candidateProfile.min_age}-${candidateProfile.max_age}`,
    );

    // Step 1: Get eligible candidates
    console.log(`\n${"=".repeat(80)}`);
    console.log(
      `STEP 1: Getting eligible candidates from getEligibleCandidates()`,
    );
    console.log(`${"=".repeat(80)}\n`);
    ...
    const candidateInEligible = eligibleCandidates.find(
      (c) => c.user_id === candidateId,
    );
    if (candidateInEligible) {
      ...
      if (!likedByViewer && !matchExists && !blockExists) {
        console.log(
          `   - Not in likes/matches/blocks, so must be filtered by database query`,
        );
        console.log(
          `   - This could be: university, campus, age range, gender preference, or interests`,
        );
      }
    }
    ...
    console.log(`\n${"=".repeat(80)}`);
    console.log(
      `STEP 2: Getting discover profiles (full algorithm with ranking)`,
    );
    console.log(`${"=".repeat(80)}\n`);
    ...
    const candidateInDiscover = discoverProfiles.find(
      (p) => p.user_id === candidateId,
    );
    if (candidateInDiscover) {
      console.log(`✅ Candidate IS in discover feed`);
      console.log(`   Name: ${candidateInDiscover.name}`);
      console.log(
        `   Position: #${discoverProfiles.indexOf(candidateInDiscover) + 1}`,
      );
      console.log(
        `   Compatibility Score: ${candidateInDiscover.compatibilityScore}`,
      );
      console.log(
        `   Liked By User (TIER 1): ${candidateInDiscover.likedByUser}`,
      );
    } else {
      console.log(`❌ Candidate IS NOT in discover feed`);
      console.log(
        `   But candidate was in eligible candidates: ${!!candidateInEligible}`,
      );
      ...
      if (candidateInEligible) {
        ...
        discoverProfiles.slice(0, 5).forEach((p, i) => {
          console.log(
            `   ${i + 1}. ${p.name} - Score: ${p.compatibilityScore}, Liked: ${p.likedByUser}`,
          );
        });
      }
    }
    ...
    console.log(
      `University match: ${viewerProfile.university_id === candidateProfile.university_id ? "✅" : "❌"}`,
    );
    console.log(
      `Campus match: ${viewerProfile.campus_id === candidateProfile.campus_id ? "✅" : "❌"}`,
    );
    console.log(
      `Candidate in viewer's age range (${viewerProfile.min_age}-${viewerProfile.max_age}): ${
        candidateAge >= viewerProfile.min_age &&
        candidateAge <= viewerProfile.max_age
          ? "✅"
          : "❌"
      }`,
    );
    console.log(
      `Viewer in candidate's age range (${candidateProfile.min_age}-${candidateProfile.max_age}): ${
        viewerAge >= candidateProfile.min_age &&
        viewerAge <= candidateProfile.max_age
          ? "✅"
          : "❌"
      }`,
    );
...
const defaultViewerEmail = process.argv[2] || "testuser8@njit.edu";
const defaultCandidateEmail = process.argv[3] || "jac352@njit.edu";

debugDiscoverAlgorithm(defaultViewerEmail, defaultCandidateEmail);
